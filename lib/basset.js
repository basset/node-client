...
class Basset {
  ...
  async uploadScreenshotFile({ title }, filePath) {
    if (!this.type !== 'image') {
    ...
    const sha = await generateFileHash(filePath);
    const relativePath = `${snapshot.title}${path.extname(filePath)}`;
    const file = fs.createReadStream(filePath);
    await this.client.uploadScreenshot({ title, }, relativePath, sha, file);
  }
}
